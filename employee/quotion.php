...
$todate=date("Y-m-d");
$id=$_GET['edit_id'];
$sql="select * from followup where id ='$id'";
				 $result=mysqli_query($conn,$sql);
				 while($idsh=mysqli_fetch_object($result))
				 {
				 
				 $name=($idsh->Customer_name);
				 $contactnoa=($idsh->Contact_No);
  ...
  <div class="pagetitle">
    ...
    <div class="row">
      ...
      <div class="card card-body">
       <h5 class="card-title">Create Quotation <?php echo $firstname?></h5>
			   ...
			   <table class="table table-bordered">
                <thead>
                  <tr>
					...
					<?php
					$sql="select * from quotation where contactno='$contactnoa' && date='$todate'";
				 $result=mysqli_query($conn,$sql);
				 while($idsh=mysqli_fetch_object($result))
				 {
...
if(isset($_POST['add']))
{
  ...
  $sql="insert into quotation(customername,contactno,employeename,date,brand,model,rate)
  value('$name','$contactnoa','$firstname','$todate','$branda','$modela','$ratea')";
